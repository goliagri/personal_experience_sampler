...
**Milestones 1–5 implemented** (shared core + conformance in Python and Kotlin; desktop MVP; Drive store: library-free OAuth in `desktop/pes/store/oauth.py` + REST `DriveStore` in `drive.py`; Android MVP: `android/runtime` is a pure-JVM Kotlin port of engine/sync/db/cloud with the scenario suites mirrored as JVM tests, `android/app` is the Compose client — exact alarms, notifications with conditional inline reply, Answer screen, permissions checklist, WorkManager sync, Kotlin `DriveStore` + Google Identity auth). M5 (sync hardening): conflict/lineage reconciliation, retroactive expiry, restore procedure (`Syncer.restore()` / `restore()`, §8.6; Settings → "Restore cloud from local cache" on both clients), weekly/monthly snapshots with 12+12 retention and the primary-role claim/handoff (§9; `CloudStore.delete` exists only for retention), and Drive recovery from deleted sub-folders (cached folder IDs are verified once per store instance). Still pending with the owner: M4's manual on-phone checklist, the Android OAuth client registration (needs the app SHA-1), and the manual restore drill against real Drive. Next per §17: M6 polish (mostly done on desktop) / M7 analysis package, plus release plumbing (signed APK, Obtainium). Authoritative documents:
