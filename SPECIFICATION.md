...
- Tokens are handled by Credential Manager on Android. Desktop implements the installed-app flow itself (PKCE + loopback redirect over plain HTTPS; no Google client libraries — owner's decision, 2026-08-22) and stores the token as a mode-0600 file in the local data directory. Neither tokens nor the client-secret JSON are ever written to the cloud folder, synced, or logged; the app records only the *path* to the client-secret JSON.
